...
```bash
rg -n "type:\\s*tool|field_instruments:\\s*\\[\\]" ./04tools
```
...
```bash
rg -n -L "score|scoring|threshold|decision rule" ./04tools/*.md
```

6. Field instruments lacking data quality checks
```bash
rg -n -L "data quality|validation|cross-check|skip pattern" ./05field-instruments/*.md
```

7. Coordination pages lacking duplication/gap logic
```bash
rg -n -L "duplication|gap|who.*where.*for whom|4W|5W" ./08coordination/*.md
```
