...
| `candidate_target_pages` | Wiki paths relative to vault root, or `["source_only"]`. Resolved downstream **by path** (never by ID). For `enrich-*` the target must already exist; for `create-*` it must not exist yet — see the routing contract in agent-09 / the index compiler. **The folder MUST be the canonical one for the action's page type** per `governance/schema/page-types.md`: concept→`02-concepts/`, framework→`03-frameworks/`, tool→`04-tools/`, field-instrument→`05-field-instruments/`, risk→`06-risks/`, decision-rule→`09-decision-protocols/`. Do not invent folders (e.g. `03-tasks/`, `04-decision-rules/` do not exist); the folder must match `integration_action` (a `create-concept` target cannot live in `07-known-tensions/`). The `finding_target_folder` lint rule enforces this. |
| `integration_action` | `create-concept` / `enrich-concept` / `create-framework` / `enrich-framework` / `create-tool` / `enrich-tool` / `create-risk` / `enrich-risk` / `create-known-tension` / `enrich-known-tension` / `create-decision-rule` / `enrich-decision-rule` / `source_only` / `flag-for-review`. Use `*-known-tension` (→ `07-known-tensions/`) for `TNS` objects — a tension is not a risk. |
