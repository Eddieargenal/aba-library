...
## Rule precedence
1. `governance/aba/agent-contract.md` (this page)
...
- `wiki/aba/01-sources/raw/` and `wiki/aba/01-sources/raw-content/` are ingest/support layers only.
- `wiki/` is the canonical answer layer.
- `governance/schema/` defines behavior and quality rules.
- Never use raw/raw-content files as direct answer content for domain queries.
...
4. Use `./01-sources/extracted/` pages for citation metadata and traceability.
...
- Do not fill gaps by bypassing synthesis and reading `01-sources/raw` or `01-sources/raw-content` as answer content.
