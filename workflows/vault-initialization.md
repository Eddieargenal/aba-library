...
Use this workflow when Hermes starts a new session and needs to load context from the vault.
...
### 2. Read Entry Point
Open `00_Start_Here.md` only if you need routing or rules not covered by context.md.
...
Check `memory/memory-rules.md` for current `stale_after_days` thresholds.
